...
const TRENDING_SEARCHES = [
  "iPhone 15", "PlayStation 5", "Air Fryer", "Nike Air Jordan", "OLED TV",
  "Mechanical Keyboard", "Wireless Earbuds", "Smart Watch", "Lego Sets", "Outdoor Furniture",
];

const COLLECTIONS = [
  { id: 1, title: "Summer Essentials", image: "https://picsum.photos/id/28/600/300", itemCount: 156, color: "from-yellow-400 to-orange-500" },
  { id: 2, title: "Tech Gadgets Under $100", image: "https://picsum.photos/id/48/600/300", itemCount: 89, color: "from-blue-400 to-indigo-500" },
  { id: 3, title: "Home Office Setup", image: "https://picsum.photos/id/36/600/300", itemCount: 124, color: "from-green-400 to-teal-500" },
];

const CATEGORY_IMAGES = {
  Fashion: "https://images.unsplash.com/photo-1483985988355-763728e1935b?w=500&h=500&fit=crop",
  Electronics: "https://images.unsplash.com/photo-1498049794561-7780e7231661?w=500&h=500&fit=crop",
  "Home & Kitchen": "https://images.unsplash.com/photo-1556911220-e15b29be8c8f?w=500&h=500&fit=crop",
  "Health & Fitness": "https://images.unsplash.com/photo-1576678927484-cc907957088c?w=500&h=500&fit=crop",
  Entertainment: "https://images.unsplash.com/photo-1511882150382-421056c89033?w=500&h=500&fit=crop",
};
...
const MainPage = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (currentUser._id && token) {
          const cartResponse = await fetch(`http://localhost:9999/shoppingCart?userId=${currentUser._id}`, {
  ...
  useEffect(() => {
    const handleScroll = () => {
      if (filtersRef.current) setIsSticky(window.scrollY > filtersRef.current.offsetTop);
    };
  ...
  try {
    ...
    if (!productResponse.ok) throw new Error(`Không thể lấy thông tin sản phẩm: ${await productResponse.text()}`);
    ...
    if (!cartId) {
      ...
      const createResponse = await fetch("http://localhost:9999/shoppingCart", {
        method: "POST",
        headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
        body: JSON.stringify({ products: [{ idProduct: productId, quantity: 1 }] }),
      });
      if (!createResponse.ok) throw new Error("Không thể tạo giỏ hàng mới");
      const newCart = await createResponse.json();
      setCartId(newCart._id); // Cập nhật cartId sau khi tạo
      showNotificationWithTimeout("Đã thêm vào giỏ hàng");
    } else {
      // Cập nhật số lượng nếu đã có giỏ hàng
      const cartResponse = await fetch(`http://localhost:9999/shoppingCart/${cartId}`, {
        method: "PATCH",
        headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
        body: JSON.stringify({ productId, quantity: 1 }), // Tăng số lượng lên 1
      });
      if (!cartResponse.ok) throw new Error("Không thể cập nhật giỏ hàng");
      showNotificationWithTimeout("Đã thêm vào giỏ hàng");
    }
  ...
  const scrollBanner = (direction) => {
    ...
    setCurrentBannerSlide((prev) =>
      direction === "left" ? (prev - 1 + BANNER_SLIDES.length) % BANNER_SLIDES.length : (prev + 1) % BANNER_SLIDES.length
    );
    const startAutoSlide = () => {
      intervalRef.current = setInterval(() => {
        setCurrentBannerSlide((prev) => (prev + 1) % BANNER_SLIDES.length);
        ...
        <div className="max-w-[1300px] mx-auto px-4 py-4">
          <div className="relative mb-8 rounded-xl overflow-hidden shadow-lg" ref={bannerRef}>
            <div className="absolute top-1/2 left-4 z-10">
              <button onClick={() => scrollBanner("left")} className="bg-white/30 backdrop-blur-sm hover:bg-white/50 text-white p-2 rounded-full">
                <FiChevronLeft className="h-6 w-6" />
              </button>
            </div>
            <div className="absolute top-1/2 right-4 z-10">
              <button onClick={() => scrollBanner("right")} className="bg-white/30 backdrop-blur-sm hover:bg-white/50 text-white p-2 rounded-full">
            ...
            <div className="relative h-[400px] overflow-hidden">
              {BANNER_SLIDES.map((slide, index) => (
                <div
                  key={slide.id}
                  className={`absolute inset-0 transition-opacity duration-1000 ${index === currentBannerSlide ? "opacity-100" : "opacity-0 pointer-events-none"}`}
                >
                  <div
                    className="absolute inset-0 bg-gradient-to-r opacity-80 z-10"
                    style={{ backgroundImage: `linear-gradient(to right, ${slide.color.split(" ")[1]}, transparent)` }}
                  ></div>
                  <img
          ...
          <div className="mb-8 relative">
            <div className="flex justify-between items-center mb-4">
              ...
              <div className="flex space-x-2">
                <button onClick={() => scrollCategories("left")} className="bg-white rounded-full p-2 shadow-sm hover:bg-gray-100">
                  <FiArrowLeft className="h-5 w-5 text-gray-700" />
                </button>
                <button onClick={() => scrollCategories("right")} className="bg-white rounded-full p-2 shadow-sm hover:bg-gray-100">
                  <FiArrowRight className="h-5 w-5 text-gray-700" />
                </button>
              </div>
            </div>
            <div ref={categoriesRef} className="flex overflow-x-auto scroll-smooth hide-scrollbar pb-2">
              <div className={`min-w-[150px] p-2 cursor-pointer ${selectedCategory === null ? "opacity-100" : "opacity-70"}`}
                   onClick={() => setSelectedCategory(null)}>
                <motion.div className={`bg-gradient-to-br from-[#0053A0] to-[#00438A] rounded-lg p-4 text-center h-full ${selectedCategory === null ? "ring-2 ring-[#0053A0] ring-offset-2" : ""}`}>
                  <div className="text-3xl mb-2">🔥</div>
                  <h3 className="font-medium text-white">Tất cả danh mục</h3>
                  <p className="text-xs text-white/80 mt-1">{products.length} sản phẩm</p>
                </motion.div>
              </div>
              {categories.map((category) => (
                <div key={category.id} className={`min-w-[150px] p-2 cursor-pointer ${selectedCategory === category.id ? "opacity-100" : "opacity-70"}`}
                     onClick={() => setSelectedCategory(category.id)}>
                  <motion.div className={`bg-white rounded-lg shadow-sm p-4 text-center h-full ${selectedCategory === category.id ? "ring-2 ring-[#0053A0] ring-offset-2" : ""}`}>
                    <div className="relative mb-2 h-12 w-12 mx-auto">
                      <img src={CATEGORY_IMAGES[category.name] || "/placeholder.svg"} alt={category.name}
          ...
          <div ref={filtersRef} className={`bg-white p-4 rounded-lg shadow-sm mb-6 ${isSticky ? "sticky top-0 z-20 shadow-md" : ""}`}>
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
              ...
              <div className="flex items-center w-full md:w-auto">
                ...
                <button onClick={() => { setSelectedCategory(null); setPriceRange([0, 1000]); setSortBy("featured"); }}
            ...
            {showFilters && (
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div>
                    <h3 className="font-medium text-gray-900 mb-2">Khoảng giá</h3>
                  ...
                  <div>
                    <h3 className="font-medium text-gray-900 mb-2">Tình trạng sản phẩm</h3>
                    <div className="space-y-2">
                      ...
                      <label className="flex items-center">
                        ...
                        <span className="ml-2 text-sm text-gray-700">Đã qua sử dụng</span>
                      </label>
                      <label className="flex items-center">
                        <input type="checkbox" className="rounded border-gray-300 text-[#0053A0] focus:ring-[#0053A0]" />
                        <span className="ml-2 text-sm text-gray-700">Tân trang</span>
                      </label>
                    </div>
                  </div>
                  <div>
                    <h3 className="font-medium text-gray-900 mb-2">Tùy chọn vận chuyển</h3>
                    <div className="space-y-2">
                      <label className="flex items-center">
                        <input type="checkbox" className="rounded border-gray-300 text-[#0053A0] focus:ring-[#0053A0]" defaultChecked />
                        <span className="ml-2 text-sm text-gray-700">Miễn phí vận chuyển</span>
                      </label>
                      <label className="flex items-center">
                        <input type="checkbox" className="rounded border-gray-300 text-[#0053A0] focus:ring-[#0053A0]" />
                        <span className="ml-2 text-sm text-gray-700">Giao hàng trong ngày</span>
                      </label>
                      <label className="flex items-center">
                        <input type="checkbox" className="rounded border-gray-300 text-[#0053A0] focus:ring-[#0053A0]" />
                        <span className="ml-2 text-sm text-gray-700">Miễn phí đổi trả</span>
                      </label>
                    </div>
                  </div>
                </div>
          ...
          <div className="mb-12">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-2xl font-bold">
                {selectedCategory ? `Sản phẩm ${getCategoryName(selectedCategory)}` : "Tất cả sản phẩm"}
              </h2>
              <div className="text-sm text-gray-500">{filteredProducts.length} kết quả</div>
            </div>
            ...
            {loading ? (
              ...
              <div className="bg-white rounded-lg shadow-sm p-8 text-center">
                ...
                <p className="text-gray-500">Không có sản phẩm nào phù hợp với tiêu chí của bạn.</p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-8">
                {paginatedProducts.map((product) => (
                  <motion.div key={product._id} className="bg-white rounded-lg shadow-sm overflow-hidden flex flex-col h-full"
                              initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                    <div className="relative w-full h-48">
                      <img src={product.url ? `${product.url}/300` : "/placeholder.svg"} alt={product.title}
                           className="w-full h-full object-cover"
                           onError={(e) => (e.target.src = "/placeholder.svg")} />
                      <button onClick={() => toggleWishlist(product)}
                              className="absolute top-2 right-2 bg-white/80 p-1.5 rounded-full hover:bg-white">
                        <FiHeart className={`h-5 w-5 ${wishlist.some((item) => item.id === product._id) ? "text-[#e43147] fill-[#e43147]" : "text-gray-600"}`} />
                      </button>
                    </div>
                    <div className="p-4 flex-grow flex flex-col">
                      <h3 className="font-medium text-gray-900 mb-1 line-clamp-2 flex-grow">{product.title}</h3>
                      ...
                      <div className="flex items-center text-xs text-gray-500 mb-2">
                        <div className="flex items-center text-yellow-400 mr-1">
                          {Array.from({ length: 5 }).map((_, i) => (
                            <FiStar key={i} className={`h-3 w-3 ${i < 4 ? "fill-yellow-400" : ""}`} />
                          ))}
                        </div>
                        <span>(1000)</span>
              ...
              <div className="space-y-4 mb-8">
                {paginatedProducts.map((product) => (
                  <motion.div key={product._id} className="bg-white rounded-lg shadow-sm overflow-hidden"
                              initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            ...
            {totalPages > 1 && (
              <div className="flex justify-center items-center gap-2 py-8">
                <button onClick={goToPreviousPage} disabled={currentPage === 1}
                        className={`flex items-center px-4 py-2 rounded-full ${currentPage === 1 ? "bg-gray-50 text-gray-400 cursor-not-allowed" : "bg-white hover:bg-gray-50 text-gray-700 border shadow-sm"}`}>
                  <FiChevronLeft className="mr-1" size={12} /> Trước
                </button>
                <div className="hidden md:flex items-center">
                  {Array.from({ length: totalPages }).map((_, index) => {
                    if (totalPages <= 7 || index === 0 || index === totalPages - 1 || (index >= currentPage - 2 && index <= currentPage + 2)) {
                      return (
                        <button key={index + 1} onClick={() => paginate(index + 1)}
                                className={`w-10 h-10 mx-1 rounded-full ${currentPage === index + 1 ? "bg-[#0053A0] text-white" : "bg-white hover:bg-gray-50 text-gray-700 border shadow-sm"}`}>
                          {index + 1}
                        </button>
                      );
                ...
                <div className="md:hidden flex items-center">
                  <span className="text-gray-600 text-sm">Trang {currentPage} / {totalPages}</span>
                </div>
                <button onClick={goToNextPage} disabled={currentPage === totalPages}
                        className={`flex items-center px-4 py-2 rounded-full ${currentPage === totalPages ? "bg-gray-50 text-gray-400 cursor-not-allowed" : "bg-white hover:bg-gray-50 text-gray-700 border shadow-sm"}`}>
          ...
          <div className="mb-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {COLLECTIONS.map((collection) => (
                <motion.div key={collection.id} className="relative rounded-lg overflow-hidden shadow-lg h-48">
                  <div className={`absolute inset-0 bg-gradient-to-r ${collection.color} opacity-80`}></div>
                  <img src={collection.image || "/placeholder.svg"} alt={collection.title}
                       className="w-full h-full object-cover"
                       onError={(e) => (e.target.src = "/placeholder.svg")} />
                  <div className="absolute inset-0 flex flex-col justify-end p-6">
                    <h3 className="text-white text-xl font-bold mb-1">{collection.title}</h3>
                    <p className="text-white/90 text-sm mb-3">{collection.itemCount} sản phẩm</p>
          ...
          <div className="mb-12">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4">
              {FEATURED_BRANDS.map((brand) => (
                <motion.div key={brand.id} className="bg-white rounded-lg shadow-sm p-4 text-center cursor-pointer">
            ...
            <div className="bg-gradient-to-r from-[#0053A0] to-[#00438A] rounded-lg p-6 text-white">
              <div className="flex flex-col md:flex-row items-center justify-between">
                <div className="mb-6 md:mb-0">
                  ...
                  <p className="text-white/80">Đăng ký nhận bản tin để nhận các ưu đãi cá nhân hóa qua email.</p>
                </div>
